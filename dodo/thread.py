...
class ThreadModel(QAbstractItemModel):
    ...
    def iterate_indices(self, start_idx: QModelIndex=QModelIndex()) -> Iterable[QModelIndex]:
        if start_idx.isValid():
            yield start_idx
        for i,c in enumerate(self._children_at(start_idx)):
            yield from self.iterate_indices(self.createIndex(i, 0, c))
    ...
    def next_unread(self, current: QModelIndex) -> QModelIndex:
        ...
        for idx in self.iterate_indices(current):
